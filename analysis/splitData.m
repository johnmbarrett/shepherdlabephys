...
%     parser.AddParameter('Indices',NaN,@(x) iscell(x) && all(cellfun(@(y) validateattributes(y,{'numeric'},{'real' 'finite' 'positive' 'integer' 'vector'}) && max(y(:)) <= size(traces,1),x)));
    ...
    parser.addParameter('Window',0.05,isRealFinitePositiveNumericScalar);
    parser.addParameter('Interval',1,isRealFinitePositiveNumericScalar);
    parser.addParameter('Start',0,isRealFinitePositiveNumericScalar);
    parser.addParameter('N',1,@(x) validateattributes(x,{'numeric'},{'real' 'finite' 'positive' 'scalar' 'integer'}));
