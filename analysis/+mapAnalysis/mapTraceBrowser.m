...
% -------------------------------------------------------------
    ...
    methods(Access=protected)
        function createFigure(self)
            ...
            numTraces = self.Recording.NMapLocations;
            numTrials = self.Recording.NTrials;
            numChannels = self.Recording.NChannels;
            ...
            self.TrialSlider = uicontrol(self.Figure, 'Tag', 'trialSlider', 'Style', 'slider', ...
                'min', 1, 'max', numTrials, ...
                'SliderStep', [1/(numTrials-1) 1/(numTrials-1)], ...
                'Callback', @self.changeTrial, 'Value', 1, ...
                'Units', 'normalized', 'Position', [0.5   0.72    0.1    0.04]);

            self.TrialEdit = uicontrol(self.Figure, 'Tag', 'trialEdit', 'Style', 'edit', ...
                'min', 1, 'max', 1, 'BackgroundColor', 'w', 'FontSize', 12, ...
                'Callback', @self.changeTrial, 'String', 1, ...
    ...
    methods
        ...
        function data = getData(self)
            % TODO : wait so we've got a map *and* a recording?  jesus h.
            % christ this is a fucking mess
            if isempty(self.Map)
                data = [];
                return
            end
