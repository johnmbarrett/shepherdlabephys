function histoN = array2Dplot(self,recording,mapNumber,isEP) % TODO : get rid of map number
    % array2Dplot
    %
    ...
    subplotRows = 5-3*isEP; subplotCols = 4-isEP; plotnum = 0;
    ...
    if isEP
        hsub(plotnum) = subplot(subplotRows,subplotCols,plotnum);
        plotTraces(hsub(plotnum), xTimeAxis, data, self.sampleRate, 'StimStart', [self.stimOnEP self.stimOnEP+self.responseDurEP]);
        set(gca, 'XLim', [0 max(xTimeAxis)]);
        line(xlim,[1 1]*self.apThreshold,'Color','k','LineStyle','--');
        xlabel('Seconds');
        ylabel('Amplitude');
        
        plotnum = plotnum+1;
        hsub(plotnum) = subplot(subplotRows,subplotCols,plotnum);
        plotMap(hsub(plotnum), recording.ActionPotentialNumber.Array, ...
                    'ColorBarText', '#',                    ...
                    'Title',        'Number of APs',        ...
        ...
        plotnum = plotnum+1;
        hsub(plotnum) = subplot(subplotRows,subplotCols,plotnum);
        plotMap(hsub(plotnum), recording.ActionPotentialLatency.Array, ...
                    'ColorBarText', '(sec)',                ...
                    'Title',        'AP Onset',             ...
                    'XData',        [-magFacX magFacX],     ...
                    'YData',        [-magFacY magFacY],     ...
